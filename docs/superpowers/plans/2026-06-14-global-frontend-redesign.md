...
- Treat baseline screenshot generation as environment-dependent; use approved writable destinations.
...
- Update `docs/DECISIONS.md` or add a focused decision file after the first design-system phase lands.
...
- Generate or collect baseline evidence without overwriting unrelated evidence.
...
- [ ] Try mocked capture into a new evidence directory:
...
- [ ] If Playwright returns `EPERM`, record the blocker and retry in a user-approved writable path.
...
- Current environment may block Playwright/Node screenshot writes.
- Visual smoke scripts can mutate invoices/payments and must not be used without disposable-target approval.
...
- Keep one visual language: institutional neutral base, restrained teal accent, semantic status tokens.
...
- Update `docs/DECISIONS.md` or a focused decision file with final design-system decision.
...
- **ALTA:** Visual evidence scripts can overwrite tracked screenshots if not configured carefully. Use intentional output destinations.
- **ALTA:** Invoice history custom action menu has accessibility issues. Must be fixed before final a11y approval.
- **ALTA:** User/password forms need full error announcement semantics. Must be fixed in relevant phases.
- **ALTA:** Warning color contrast must be checked in light mode.
