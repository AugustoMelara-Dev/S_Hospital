...
* **E2E reproducible:** `test:e2e:mock` aprobó inicialmente **19/19** y, tras incorporar Respaldos, Ayuda, Soporte y Acerca de, cerró en **21/21** sin secretos. `test:e2e:release` exige `E2E_RELEASE_PASSWORD` o `E2E_SEED_PASSWORD` y falla de forma explícita si faltan.
...
* **Respaldos:** migrado después del gate: `backupAntd.tsx` eliminado; Ant Design Card/Empty/Alert/Modal/Pagination/Statistic/Tag directos e InstitutionalDataGrid preservado. Vitest **55/55**, Playwright mock **1/1**, typecheck y lint aprobados. Estado: IMPLEMENTADA — QA TRANSVERSAL PENDIENTE.
* **Ayuda, Soporte y Acerca de:** eliminados los adaptadores locales que imitaban primitivas anteriores (`supportAntd.tsx` y `aboutAntd.tsx`). Las vistas usan Ant Design, Ant Design Icons, `PageHeader` y `StatusTag` institucionales directamente. Vitest **14/14**, Playwright Chromium **1/1**, typecheck, lint y gate estricto aprobados. Estado: IMPLEMENTADA — QA TRANSVERSAL PENDIENTE.
