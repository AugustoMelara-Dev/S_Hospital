...
| Current status | Independent review complete on `codex/v1-1-polish-review`; merged to `main` at `7c49592968850cd890b6781788b42cf7a1273fd8`, physical production not approved |
...
| B - Invoice, Payment & Receipt Premium UX | `codex/v1-1-invoice-receipt-premium` | `C:\Projects\S_Hospital-v1-1-invoice-receipt` | `frontend/src/features/invoices/**`, `frontend/src/features/receipts/**`, receipt print CSS, receipt-focused tests, `docs/ux/INVOICE_RECEIPT_PREMIUM_REVIEW.md` | Money math, reducer semantics, idempotency, CAI/series numbering, historical snapshots, permissions | None approved | Integrated on base branch; mobile POS total slice completed and reviewed | `npm.cmd run test -- InvoiceConfirmation.test.tsx --run`; `npm.cmd run test -- NewInvoiceViewLayout.test.tsx InvoiceCart.test.tsx --run`; `npm.cmd run typecheck`; `npm.cmd run lint` | Receipt polish can accidentally invent fields or imply physical print approval |
| C - Reports & Analytics Premium | `codex/v1-1-reports-analytics-premium` | `C:\Projects\S_Hospital-v1-1-reports` | `frontend/src/features/reports/**`, dashboard report cards, chart/test files, `docs/ux/REPORTS_ANALYTICS_PREMIUM_REVIEW.md` | Backend report contracts, export calculations, permissions, invented KPIs | None approved | Explorer completed: `019effb2-1ccc-7923-97cf-42bce747de28`; orchestrator tab/filter and chart-token slices completed and reviewed | `npm.cmd test -- --run src/features/reports/ReportsView.test.tsx src/features/reports/components/IncomeReportTab.test.tsx src/features/reports/components/CashSessionReportTab.test.tsx`; `npm.cmd run typecheck`; `npm.cmd run lint` | Dense reports may hide data context; chart changes must keep accessibility and dark mode |
...
| E - Operations UX: CashBox, Backups, Catalog, Settings | `codex/v1-1-operations-polish` | `C:\Projects\S_Hospital-v1-1-operations` | `frontend/src/features/cashbox/**`, `frontend/src/features/catalog/**`, `frontend/src/features/backups/**`, `frontend/src/features/settings/**`, `frontend/src/features/receipt-settings/**`, `docs/ux/OPERATIONS_UX_REVIEW.md` | Cash close math/contracts, backup/restore behavior, fiscal numbering semantics, permissions | None approved | Explorer completed: `019effb2-43fc-78e2-9d75-2aac79d11cd4`; orchestrator receipt-settings slice completed and reviewed | `npm.cmd run test -- receipt-settings --run`; `npm.cmd run typecheck`; `npm.cmd run lint` | Settings polish can accidentally imply legal data or alter sequence behavior |
| F - Accessibility, Responsive, Performance & QA | `codex/v1-1-a11y-responsive-qa` | `C:\Projects\S_Hospital-v1-1-qa` | Playwright/e2e specs, axe tests, screenshots, `docs/qa/V1_1_POLISH_QA_REPORT.md`, `docs/qa/V1_1_PERFORMANCE_LAN_REVIEW.md`, `docs/qa/V1_1_RECEIPT_PDF_DIGITAL_PROOF.md`, `docs/qa/V1_1_MYSQL_MARIADB_FOCAL_PROOF.md` | Production data, backend contracts, business logic | None approved | Independent review complete: visual review, full route axe/a11y matrix, frontend gates, backend partial Docker evidence, release E2E, receipt PDF digital proof, security/RBAC review, disposable MariaDB focal proof, and final post-merge backend gate captured | `npx playwright test e2e/v1-1-full-a11y.spec.ts`: 7 passed; `npm.cmd run smoke:buttons`: 7 passed; `npm.cmd run test`: 82 files/487 tests passed; backend partial Docker evidence: exit 0, 49 passed, 668 warnings, 1 skipped, 4672 assertions; final post-merge backend gate: exit 0, 707 passed, 11 skipped, 4672 assertions; `npm.cmd run build`: passed; release E2E: 2 passed, 0 skipped/unexpected/flaky; `InstitutionalReceiptPdfTest`: 13 tests/171 assertions passed with Docker `.env` warnings; MariaDB focal: 71 tests/614 assertions passed | Visual/performance evidence is digital and mocked/local only until real LAN/printer/restore/load validation; physical production remains not approved |
...
- Backend partial Docker evidence from the polish branch passed when Docker mounted the full repo root at `/workspace` with the backend vendor volume; the earlier Compose-only backend mount was insufficient for repo-root guard tests.
- Release E2E is now green through a Docker-backed Laravel server plus Vite; Playwright release artifacts are isolated under `frontend/test-results/release-e2e-artifacts` to avoid Windows cleanup blocking on live service logs.
- Receipt PDF digital proof is now recorded in `docs/qa/V1_1_RECEIPT_PDF_DIGITAL_PROOF.md`; it does not approve physical printer output.
- Focused successful admin mobile reports evidence is now present at `qa/screenshots/v1-1-production-polish/mobile-reports-admin-light.png`.
- Disposable MariaDB focal proof is now recorded in `docs/qa/V1_1_MYSQL_MARIADB_FOCAL_PROOF.md`; it found and fixed a test collation issue around `Admin`/`admin` role names, then passed 71 tests and 614 assertions.
- Independent review branch added final full axe/security review and found no digital P0/P1 blocker for internal merge.
- Final post-merge backend gate on `main` passed with exit code 0: 707 passed, 11 skipped, 4672 assertions.
- Next safe candidates are real LAN/printer/restore/load validation after merge readiness; do not claim physical production complete until those operational proofs exist.
