...
| F - Accessibility, Responsive, Performance & QA | `codex/v1-1-a11y-responsive-qa` | `C:\Projects\S_Hospital-v1-1-qa` | Playwright/e2e specs, axe tests, screenshots, `docs/qa/V1_1_POLISH_QA_REPORT.md`, `docs/qa/V1_1_PERFORMANCE_LAN_REVIEW.md`, `docs/qa/V1_1_RECEIPT_PDF_DIGITAL_PROOF.md`, `docs/qa/V1_1_MYSQL_MARIADB_FOCAL_PROOF.md` | Production data, backend contracts, business logic | None approved | Partial mocked visual, responsive axe smoke, full frontend unit/component, backend Docker test, frontend build performance, Docker-backed release E2E, receipt PDF digital proof, focused admin mobile reports evidence, and disposable MariaDB focal proof captured by orchestrator | `npx.cmd playwright test e2e/production-readiness.spec.ts` with `E2E_CAPTURE_RC_SCREENSHOTS=1`: 4 passed, 33 screenshots, 0 console issues; focused responsive admin mobile reports pass: 1 test, 0 console issues, 1 added screenshot; `npm.cmd run smoke:buttons`: 7 passed; `npm.cmd run test`: 82 files/487 tests passed; Docker `php artisan test --colors=never`: 49 passed, 668 warnings, 1 skipped; `npm.cmd run build`: passed; release E2E: 2 passed, 0 skipped/unexpected/flaky; `InstitutionalReceiptPdfTest`: 13 tests/171 assertions passed with Docker `.env` warnings; MariaDB focal: 71 tests/614 assertions passed | Visual/performance evidence is digital and mocked/local only until real LAN/printer/restore/load validation; full restore/load validation still needs final evidence |
...
- Next safe candidates are real backend/LAN/printer/restore/load validation and final full axe/security review.
