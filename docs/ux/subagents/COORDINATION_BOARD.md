...
| C - Reports & Analytics Premium | `codex/v1-1-reports-analytics-premium` | `C:\Projects\S_Hospital-v1-1-reports` | `frontend/src/features/reports/**`, dashboard report cards, chart/test files, `docs/ux/REPORTS_ANALYTICS_PREMIUM_REVIEW.md` | Backend report contracts, export calculations, permissions, invented KPIs | None approved | Explorer completed: `019effb2-1ccc-7923-97cf-42bce747de28`; orchestrator tab/filter slice in progress | `npm.cmd test -- --run src/features/reports/ReportsView.test.tsx src/features/reports/components/IncomeReportTab.test.tsx src/features/reports/components/CashSessionReportTab.test.tsx`; `npm.cmd run typecheck`; `npm.cmd run lint` | Dense reports may hide data context; chart changes must keep accessibility and dark mode |
...
- Reports tab/filter polish is being addressed on the base polish branch: labeled report tabs, accent-clean tab labels, wrapping filter controls in daily/cash/service tabs.
- Next safe candidates are report chart summaries/colors and POS mobile total visibility.
