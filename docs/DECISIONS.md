...
visible para abrir el sistema y puede ejecutarse durante soporte o
capacitacion, pero su salida solo ocultaba secretos y rutas Windows.

Decision: `Protect-ShortcutText` tambien reemplaza rutas locales Unix y
...
esos patrones antes de ejecutar el dry-run del acceso directo.
...
## 2026-06-08 - Autoarranque Windows protege salida

Contexto: `install_stack_autostart_windows.ps1` registra el
autoarranque del stack en Windows y su salida puede usarse como
evidencia de soporte, pero su sanitizacion no cubria rutas absolutas
Unix ni XML crudo de tareas.

Decision: `Protect-AutostartText` reemplaza rutas locales Unix y
fragmentos XML de tareas por `[ruta-local]` y `[xml-protegido]`.
`validate_startup_repair_safety.ps1` protege esos patrones y mantiene
los dry-runs no destructivos del autoarranque.
...
## 2026-06-08 - guard de Nueva factura cubre Todos y parciales
...
## 2026-06-08 - Usuarios de area consultan servicios pagados sin modulo clinico
...
## 2026-06-08 - Matriz protege alcance minimo del rol de area
Contexto: el rol `area` ya existia con un unico permiso operativo, pero la matriz principal de permisos no mostraba explicitamente que no puede operar caja, facturar, cobrar, anular, reimprimir, ver reportes gerenciales, administrar usuarios ni gestionar respaldos.
Decision: `docs/PERMISSIONS_MATRIX.md` agrega la columna `area` en la matriz principal y mantiene la consulta de servicios pagados como la unica capacidad permitida para ese rol. `validate_permission_audit_safety.ps1` valida que el seeder del rol `area` conserve solo `areas.paid_services.view` y que la matriz documente sus restricciones.
Criterio de verificacion: `scripts\validate_permission_audit_safety.ps1` debe reportar `PERMISSION_AUDIT_SAFETY: YES` y fallar si el rol `area` recibe permisos de caja, facturacion, pagos, recibos, reportes, usuarios, respaldos, configuracion, catalogo o auditoria.

## 2026-06-08 - Paquete offline exige manual de usuario de area
Contexto: el rol `area` ya tenia pantalla, evidencia visual y manual operativo, pero el constructor del paquete offline solo trataba como manuales criticos a cajero, supervisor y administrador. Eso podia entregar una RC sin instrucciones claras para laboratorio, rayos X u otras areas que consultan servicios pagados.
Decision: `scripts\make_offline_release.ps1` incluye `manuales\MANUAL_USUARIO_AREA.md` en la lista de documentos criticos, y `scripts\assert_offline_release_clean.ps1` exige y compara los manuales criticos contra la fuente versionada.
Criterio de verificacion: `scripts\make_offline_release.ps1 -SelfTest`, `scripts\assert_offline_release_clean.ps1 -SelfTest`, `scripts\validate_operator_manuals_safety.ps1` y `scripts\assert_offline_release_clean.ps1 -RequireCurrentCommit` deben fallar si el manual de area falta o queda desfasado en el paquete offline.

## 2026-06-08 - Indice documental no declara READY sin campo
Contexto: `docs\00_README.md` es la puerta de entrada a la documentacion y podia confundir dos puntos: no listaba el manual de usuario de area y describia el handoff como `PRODUCTION_READY=YES` aunque faltan evidencias fisicas finales.
Decision: el indice enlaza `manuales\MANUAL_USUARIO_AREA.md` y describe el handoff como `PRODUCTION_CANDIDATE` hasta completar LAN, impresora, autoarranque, respaldos y capacitacion. `validate_operator_manuals_safety.ps1` protege ese texto junto con el resto de manuales por rol, y el paquete offline trata `docs\00_README.md` como documento critico comparado contra Git.
Criterio de verificacion: `scripts\validate_operator_manuals_safety.ps1`, `scripts\make_offline_release.ps1 -SelfTest`, `scripts\assert_offline_release_clean.ps1 -SelfTest`, `scripts\validate_final_handoff_completeness.ps1`, `scripts\validate_operations_objective_audit.ps1` y `scripts\check-branding.ps1` deben pasar sin que el indice principal vuelva a prometer produccion lista antes de evidencia de campo.

## 2026-06-08 - Indice documental enumera todos los proofs finales
Contexto: `docs\00_README.md` ya habia dejado de declarar READY, pero su seccion de evidencia aun hablaba de "four PROOF files" y no listaba autoarranque, respaldo final ni capacitacion supervisada, aunque esos archivos bloquean el preflight real.
Decision: el indice lista los siete proofs finales requeridos: LAN cliente, impresion institucional, autoarranque, respaldo final, restore, concurrencia y capacitacion. El texto del preflight habla de cualquier proof final requerido, no de una cantidad fija.
Criterio de verificacion: `scripts\validate_operator_manuals_safety.ps1` debe fallar si `docs\00_README.md` omite alguno de los siete proofs o vuelve a decir "four PROOF files"; `scripts\assert_offline_release_clean.ps1 -RequireCurrentCommit` debe comparar el indice corregido dentro del paquete offline.

## 2026-06-08 - Release gate exige todos los proofs finales
Contexto: el preflight local bloqueaba siete comprobantes finales, pero `.github\workflows\release.yml` y `docs\CI.md` seguian describiendo solo cuatro evidencias fisicas. Eso permitia que un tag de release quedara desalineado con los bloqueantes reales de LAN, impresora, autoarranque, respaldos, restore, concurrencia y capacitacion.
Decision: el workflow de release valida que existan los siete archivos finales y que ninguno siga en PENDING antes de crear un GitHub Release; la documentacion de CI enumera la misma lista. `validate_production_ready_gate_safety.ps1` lee ambos archivos y falla si falta un proof, si el workflow no rechaza archivos ausentes o si reaparece el lenguaje de cuatro evidencias.
Criterio de verificacion: `scripts\validate_production_ready_gate_safety.ps1`, `scripts\validate_final_handoff_completeness.ps1`, `scripts\validate_operations_objective_audit.ps1` y `scripts\check-branding.ps1` deben pasar; el paquete offline se regenera despues del commit para incluir la documentacion y guard actualizados.

## 2026-06-08 - Notas finales no subcuentan evidencia fisica
Contexto: las notas operativas activas y las release notes finales aun listaban solo LAN, impresion, restore y concurrencia como pasos fisicos, aunque el preflight real tambien bloquea autoarranque, respaldo final y capacitacion supervisada.
Decision: `docs\OPERATIVE_NOTES_2026_06_02.md` y `RELEASE_NOTES_v1.0.0_FINAL.md` enumeran los siete proofs finales. `validate_production_ready_gate_safety.ps1` ahora falla si esos documentos omiten cualquier proof o vuelven a lenguaje de cuatro evidencias.
Criterio de verificacion: `scripts\validate_production_ready_gate_safety.ps1`, `scripts\validate_final_handoff_completeness.ps1`, `scripts\validate_operations_objective_audit.ps1`, `scripts\validate_operator_manuals_safety.ps1` y `scripts\check-branding.ps1` deben pasar antes de generar el paquete offline.

## 2026-06-08 - Audit activo refleja siete evidencias fisicas
Contexto: `docs\AUDIT_2026_06_02.md` sigue enlazado como baseline y plan vigente desde el indice documental, pero su seccion de PRODUCTION_READY fisico todavia agrupaba el cierre como cuatro evidencias mas worker/handoff.
Decision: el audit mantiene su numeracion historica de fases, pero FASE 5 cubre autoarranque y respaldo final, FASE 6 cubre capacitacion y handoff, y el mapa operativo nombra los siete proofs finales. `validate_production_ready_gate_safety.ps1` ahora lee el audit activo y falla si omite cualquier proof final.
Criterio de verificacion: `scripts\validate_production_ready_gate_safety.ps1`, `scripts\validate_operations_objective_audit.ps1`, `scripts\validate_operator_manuals_safety.ps1` y `scripts\check-branding.ps1` deben pasar sin que `docs\AUDIT_2026_06_02.md` vuelva a subcontar evidencia fisica.

## 2026-06-08 - Checklist final cubre capacitacion de usuario de area
Contexto: el checklist de release y la prueba final de capacitacion cubrian cajero, supervisor y administrador, pero podian omitir el rol `area`, aunque el sistema ya incluye usuarios de laboratorio/rayos X que consultan servicios pagados sin operar caja ni expediente clinico.
Decision: `docs\RELEASE_CHECKLIST.md`, `qa\TRAINING_ACCEPTANCE_PROOF.example.md` y `qa\TRAINING_ACCEPTANCE_PROOF.md` incluyen usuario de area. `validate_training_acceptance_proof.ps1`, `validate_proof_initialization_safety.ps1` y `validate_production_ready_gate_safety.ps1` fallan si la evidencia o el checklist omiten ese proof/rol final.
Criterio de verificacion: `scripts\validate_training_acceptance_proof.ps1 -AllowPendingFinalField`, `scripts\validate_proof_initialization_safety.ps1`, `scripts\validate_production_ready_gate_safety.ps1`, `scripts\validate_installation_docs_safety.ps1`, `scripts\validate_operator_manuals_safety.ps1` y `scripts\check-branding.ps1` deben pasar antes de regenerar el paquete offline.

## 2026-06-08 - Preflight final exige capacitacion de usuario de area
...
## 2026-06-08 - Handoff final apunta al indice de bloqueantes
...
## 2026-06-08 - Plantillas finales piden evidencia anonima
Contexto: el indice final exigia evidencia anonima y sin nombres de personal, pero varias plantillas finales pedian `Responsible person` sin aclarar que soporte debe usar rol, cargo o iniciales en vez de nombres completos.
Decision: las siete plantillas finales conservan los nombres de campo que valida el preflight, pero agregan instruccion uniforme de anonimizar evidencia, usar rol/cargo/iniciales y no escribir nombres completos, pacientes, usuarios, passwords, `.env` ni rutas locales. `validate_field_proof_templates.ps1` y `validate_final_field_blockers_safety.ps1` protegen esa regla.
Criterio de verificacion: `scripts\validate_field_proof_templates.ps1`, `scripts\validate_final_field_blockers_safety.ps1`, `scripts\validate_proof_initialization_safety.ps1`, `scripts\validate_production_ready_gate_safety.ps1` y `scripts\check-branding.ps1` deben pasar antes de regenerar el paquete offline.

## 2026-06-08 - Preflight rechaza restore y concurrencia locales como finales
Contexto: `qa\FINAL_RESTORE_PROOF.md` y `qa\FINAL_CONCURRENCY_PROOF.md` conservan evidencia local util, pero el preflight final podia validar campos/checks sin rechazar marcadores como Docker/MariaDB development, localhost o Target environment local.
Decision: `production_readiness_preflight.ps1` rechaza esos marcadores locales para los proofs finales de restore y concurrencia, de modo que el cierre real exija evidencia del servidor final o destino descartable final. `validate_production_ready_gate_safety.ps1` protege esos patrones.
Criterio de verificacion: `scripts\validate_production_ready_gate_safety.ps1`, `scripts\validate_final_field_blockers_safety.ps1`, `scripts\validate_operations_objective_audit.ps1` y `scripts\check-branding.ps1` deben pasar antes de regenerar el paquete offline.
