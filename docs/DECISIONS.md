...
### 2026-05-31 - GET concurrente y errores visibles en cambio de contrase帽a
...
- Los usuarios con contrase帽a temporal necesitan ver errores de validacion sin depender solo de la barra de estado del shell.

Consecuencia:

- La carga de vistas con varias consultas mejora sin relajar seguridad de mutaciones.
- Errores 422 de cambio de contrase帽a se muestran en la pantalla activa.
...
- El usuario reporto recargas que quedaban en blanco e imposibilidad de iniciar sesion; el primer rescate debe proteger el flujo de acceso antes de ampliar el redise帽o.
...
### 2026-05-31 - Reversi贸n auditable de pagos

Decision:

- Los pagos se corrigen mediante reversi贸n auditable (`status=void`) y no se borran.
- Cada reversi贸n exige permiso `payments.void`, motivo de servidor, usuario, fecha y auditor铆a.
- La factura se recalcula dentro de la misma transacci贸n desde pagos `posted`, dejando `paid_amount`, `balance_due` y `status` consistentes.

Motivo:

- Caja y reportes deben excluir pagos reversados sin perder la evidencia de que el pago existi贸.
- Una correcci贸n de m茅todo o monto no debe depender de edici贸n manual ni de memoria humana.
- El arqueo debe conservar la huella del reverso con un movimiento negativo asociado al pago original.

Consecuencia:

- Los reportes existentes, que ya filtran pagos `posted`, excluyen reversos autom谩ticamente.
- El detalle de movimientos de caja puede mostrar `payment_void` para explicar por qu茅 el efectivo esperado baj贸.
- La UI puede consumir el nuevo endpoint de reversi贸n sin crear una fuente secundaria de verdad para estados de pago.
...
- La evoluci贸n por fecha se limita a d铆as con actividad financiera para evitar llenar la respuesta con filas vac铆as sin contexto operativo.

Motivo:

- Administraci贸n necesita lectura mensual de facturado, cobrado, pendiente, parcial, anulado y m茅todos de pago sin sumar manualmente reportes diarios.
- El mensual debe excluir cobros de facturas anuladas y mantener la misma sem谩ntica que los reportes existentes.

Consecuencia:

- La UI puede consumir un contrato mensual estable sin recalcular dinero en frontend.
- Las pruebas comparan d铆as activos, m茅todos y estados para detectar divergencias entre lectura diaria y mensual.
...
- La pesta帽a frontend de servicios usa "facturado" para sus KPI, tablas, graficas y estados vacios cuando consume reportes de servicios/categorias.
...
- La se帽al ayuda al diagnostico local, pero no sustituye la prueba fisica desde una segunda PC LAN.
...
- payments a馻de columna mount_cents bigInteger (migration 2026_06_01_000001).
...
- invoices y invoice_items aun usan decimal(12,2) sin columna cents. Sus ROUND(total * 100) y ROUND(line_total * 100) en report services quedan. Migrarlos requiere a馻dir *_cents a las dos tablas y backfill, que es scope de una iteracion futura.
...
- parseCents, formatCents, parseQuantityUnits estaban duplicados en NewInvoiceView, InvoiceCart, PaymentModal, CashBoxView, OpenSessionForm. Cada copia tenia peque馻s variaciones de regex/precision, lo que hacia que el redondeo de UI difiriera entre vistas.
...
- docker-compose.prod.yml a馻de healthcheck al backend (DB::connection()->getPdo() via tinker) y a nginx (wget http://localhost/up).
...
- El limite de 100M era enga駉so: nginx aceptaba el body pero PHP lo rechazaba con un 413/500 silencioso.
...
- La pesta馻 de Auditoria muestra `catalog_changes` con etiquetas humanas: servicio, tipo de cambio, antes, despues, motivo, usuario y fecha.
...
## 2026-06-14 - Phase 7: Historial, recibos y reimpresi贸n

- Se refin贸 el flujo de historial de facturas, recibos y reimpresi贸n sin cambiar contratos API ni reglas de auditor铆a.
- La anulaci贸n autorizada qued贸 como acci贸n directa visible con confirmaci贸n auditada; se elimin贸 el men煤 manual previo para mejorar teclado, claridad y pruebas.
- `ConfirmDialog` conserva Radix AlertDialog, pero separa la descripci贸n accesible del contenido visual para evitar controles interactivos dentro de `AlertDialog.Description`.
- Se normaliz贸 copy institucional en impresi贸n, reimpresi贸n, anulaci贸n y recibos con acentos correctos, manteniendo tama帽os de recibo existentes.