...
$lanClientStandaloneValidatorPath = Join-Path $scriptRoot "validate_lan_client_standalone.ps1"
$physicalPrintProofPath = Join-Path $scriptRoot "register_physical_receipt_print_proof.ps1"
...
Assert-Contains $preflightPath 'Test-InstitutionalPrinterProofFile' "production_readiness_preflight.ps1 debe usar una validacion dedicada para recibo institucional fisico."
Assert-Contains $preflightPath 'at least one checked physical institutional paper item' "production_readiness_preflight.ps1 debe aceptar el papel institucional final real sin exigir media carta+carta+A5 simultaneamente."
Assert-Contains $preflightPath 'photo, signed note, printed sample, or real printer' "production_readiness_preflight.ps1 debe exigir evidencia fisica, no solo PDF virtual."
...
Assert-Contains $lanClientStandaloneValidatorPath 'Standalone PowerShell validation' "validate_lan_client_standalone.ps1 debe generar evidencia sin depender del repo en segunda PC."
Assert-Contains $lanClientStandaloneValidatorPath 'WebSocket TCP' "validate_lan_client_standalone.ps1 debe verificar el puerto WebSocket/Soketi desde segunda PC."
Assert-Contains $lanClientStandaloneValidatorPath 'Manual browser workflow from this second PC' "validate_lan_client_standalone.ps1 debe dejar checklist manual de flujo real en navegador."
Assert-Contains $lanClientStandaloneValidatorPath 'Server LAN URL: $BaseUrl' "validate_lan_client_standalone.ps1 debe escribir el campo exacto que valida el preflight."
Assert-Contains $physicalPrintProofPath 'Esta acta es solo para impresora fisica real' "register_physical_receipt_print_proof.ps1 debe rechazar impresoras virtuales/PDF."
Assert-Contains $physicalPrintProofPath 'ValidateSet("media carta", "carta", "A5")' "register_physical_receipt_print_proof.ps1 debe limitar el papel principal a formatos institucionales."
Assert-Contains $physicalPrintProofPath 'Photo path, printed-sample reference, or signed local note' "register_physical_receipt_print_proof.ps1 debe dejar evidencia fisica trazable."
...
Assert-Contains $finalHandoffPath '[switch] $RequireCurrentCommit' "final_production_handoff.ps1 debe permitir exigir commit actual solo en handoff de release final, no en validaciones operativas de worktree sucio."
Assert-Contains $finalHandoffPath '$releaseGuardArgs += "-RequireCurrentCommit"' "final_production_handoff.ps1 debe pasar -RequireCurrentCommit solo cuando se solicite explicitamente."
Assert-Contains $finalHandoffPath 'Get-LanStandaloneValidationCommand' "final_production_handoff.ps1 debe reportar el comando standalone para segunda PC sin repo."
Assert-Contains $finalHandoffPath 'validate_lan_client_standalone.ps1' "final_production_handoff.ps1 debe incluir el validador standalone LAN en el handoff."
Assert-Contains $finalHandoffPath 'Get-PhysicalPrintProofCommand' "final_production_handoff.ps1 debe reportar el comando para registrar evidencia fisica de impresion."
Assert-Contains $finalHandoffPath 'register_physical_receipt_print_proof.ps1' "final_production_handoff.ps1 debe incluir el generador de acta de impresion fisica."
Assert-Contains $finalHandoffPath 'Get-HandoffRerunCommand' "final_production_handoff.ps1 debe regenerar su comando final preservando EnvFile y ComposeProjectName."
