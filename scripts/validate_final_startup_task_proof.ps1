...
if ($ProofPath -eq "") {
    $ProofPath = Join-Path $ProjectRoot "qa\FINAL_STARTUP_TASK_PROOF.md"
...
function Protect-StartupProofText([string] $value) {
...
function Add-Failure([string] $message) {
    $safe = Protect-StartupProofText $message
    $failures.Add($safe) | Out-Null
    Write-Host "[FAIL] $safe" -ForegroundColor Red
}

function Add-Pass([string] $message) {
    Write-Host "[ OK ] $(Protect-StartupProofText $message)" -ForegroundColor Green
}
...
function Test-ProofHasCompletedField([string] $content, [string] $fieldLabel) {
    ...
    if (Test-ProofValueIsIncomplete $value) {
        Add-Failure "Complete '${fieldLabel}:' in qa\FINAL_STARTUP_TASK_PROOF.md."
    } else {
...
function Test-ProofHasCheckedItem([string] $content, [string] $checkLabel) {
    ...
    if ($content -match "(?im)^\s*-\s*\[[xX]\]\s*.*$escaped.*$") {
        ...
        Add-Failure "Complete a checked evidence item for '$checkLabel' in qa\FINAL_STARTUP_TASK_PROOF.md."
    }
}
...
function Test-ProofReferencedEvidence([string] $content) {
    ...
    if (Test-ProofValueIsIncomplete $value) {
        Add-Failure "Complete 'Evidence/capture reference:' in qa\FINAL_STARTUP_TASK_PROOF.md."
        return
    }

    if ([System.IO.Path]::IsPathRooted($value)) {
        Add-Failure "Final startup task evidence must use a relative qa/ path or non-local physical/support reference, not an absolute local path."
        return
    }
    ...
    if ($looksLikeRepoPath) {
        if ($value -notmatch '^qa[\\/]' -or $value -match '(^|[\\/])\.\.([\\/]|$)') {
            Add-Failure "Final startup task evidence must reference files under qa/ without traversal, or use a non-local physical/support reference."
            return
        }

        $candidate = Join-Path $ProjectRoot $value
        if (-not (Test-Path -LiteralPath $candidate)) {
            Add-Failure "Final startup task evidence references missing local evidence '$value'."
            return
        }
    }
...
if ([string]::IsNullOrWhiteSpace($ProofPath) -or -not (Test-Path -LiteralPath $ProofPath -PathType Leaf)) {
    Add-Failure "Missing qa\FINAL_STARTUP_TASK_PROOF.md. Run scripts\init_production_proofs.ps1, then fill it after final-server startup validation."
} else {
    Add-Pass "Found qa\FINAL_STARTUP_TASK_PROOF.md"
    ...
    if ($content -match '(?i)PENDING_FINAL_FIELD') {
        if ($AllowPendingFinalField) {
            foreach ($pendingRequirement in @(
                @{ Pattern = '(?i)SistemaCajaHospitalaria-StackAutostart'; Label = 'SistemaCajaHospitalaria-StackAutostart' },
                @{ Pattern = '(?i)AtStartup'; Label = 'AtStartup' },
                @{ Pattern = '(?i)arranque|startup|reinicio|reboot'; Label = 'arranque/reinicio' },
                @{ Pattern = '(?i)/up'; Label = '/up' },
                @{ Pattern = '(?i)login'; Label = 'login' },
                @{ Pattern = '(?i)PRODUCTION_CANDIDATE'; Label = 'PRODUCTION_CANDIDATE' }
            )) {
                if ($content -match $pendingRequirement.Pattern) {
                    Add-Pass "Pending startup proof keeps blocker: $($pendingRequirement.Label)"
                } else {
                    Add-Failure "Pending startup proof must keep blocker text: $($pendingRequirement.Label)"
                }
            }
        } else {
            Add-Failure "qa\FINAL_STARTUP_TASK_PROOF.md is still PENDING_FINAL_FIELD."
        }
    }

    if ($content.Trim().Length -lt 300) {
        Add-Failure "qa\FINAL_STARTUP_TASK_PROOF.md is too short to contain final-server startup evidence."
    }
    ...
    if (-not ($AllowPendingFinalField -and $content -match '(?i)PENDING_FINAL_FIELD')) {
        foreach ($placeholder in @(
            ...
            if ($content -match $placeholder.Pattern) {
                Add-Failure "$($placeholder.Message) in qa\FINAL_STARTUP_TASK_PROOF.md."
            }
        }
    }
    ...
    foreach ($secretPattern in @(
        ...
        if ($content -match $secretPattern) {
            Add-Failure "qa\FINAL_STARTUP_TASK_PROOF.md must not expose secrets, local machine paths or task XML."
            break
        }
    }
}
...
if ($failures.Count -gt 0) {
    Write-Host ""
    Write-Host "FINAL_STARTUP_TASK_PROOF: NO ($($failures.Count) blocking issue(s))" -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "FINAL_STARTUP_TASK_PROOF: YES" -ForegroundColor Green
