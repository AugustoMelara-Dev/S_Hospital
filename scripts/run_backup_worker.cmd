...
if "%BACKUP_MODE%"=="php" (
    ...
    if defined CHECK_ONLY (
        echo Verificacion completada. El worker de respaldos puede iniciarse con PHP local sin tocar datos en este paso.
        exit /b 0
    )
...
for /f "usebackq delims=" %%S in (`docker compose %COMPOSE_PROJECT_ARGS% -f "%COMPOSE_FILE%" --env-file "%ENV_FILE%" config --services`) do (
    ...
    if /I "%%S"=="queue-worker" set "HAS_WORKER_SERVICE=1"
)
...
if not defined HAS_WORKER_SERVICE (
    echo ERROR: docker-compose.prod.yml no define el servicio queue-worker requerido para respaldos.
    exit /b 1
)
...
if defined CHECK_ONLY (
    ...
    if not defined BACKEND_CONTAINER (
        echo ERROR: El servicio Docker backend no esta activo. Inicie el sistema antes de validar el worker.
        exit /b 1
    )

    set "WORKER_CONTAINER="
    for /f %%I in ('docker compose %COMPOSE_PROJECT_ARGS% -f "%COMPOSE_FILE%" --env-file "%ENV_FILE%" ps --status running -q queue-worker 2^>nul') do set "WORKER_CONTAINER=%%I"
    if not defined WORKER_CONTAINER (
        echo ERROR: El servicio Docker queue-worker no esta activo. Inicie el sistema antes de validar el worker.
        exit /b 1
    )
    ...
    docker compose %COMPOSE_PROJECT_ARGS% -f "%COMPOSE_FILE%" --env-file "%ENV_FILE%" exec -T queue-worker php artisan --version >nul 2>>"%LOG_FILE%"
    if errorlevel 1 (
        echo ERROR: El contenedor queue-worker no responde a artisan. Revise logs antes de entregar respaldos automaticos.
        exit /b 1
    )

    echo Verificacion completada. El worker Docker de respaldos puede asegurarse sin crear respaldos.
    exit /b 0
)

echo [%date% %time%] Asegurando queue-worker Docker. >> "%LOG_FILE%"
docker compose %COMPOSE_PROJECT_ARGS% -f "%COMPOSE_FILE%" --env-file "%ENV_FILE%" up -d queue-worker >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo ERROR: No se pudo iniciar el worker Docker de respaldos. Revise install-logs\backup_worker_task.log.
    exit /b 1
)

echo Worker Docker de respaldos activo.
endlocal
