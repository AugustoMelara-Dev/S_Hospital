...
class ExecutiveExcelExportService
{
    ...
    private function buildSummarySheet(
        ...
        $rows = [
            ...
            ['Total Facturado', $this->moneyFloat($summary['billed_total'] ?? '0.00'), $this->pctLabel($comparison['billed']['delta_percentage'] ?? null)],
            ['Total Cobrado', $this->moneyFloat($summary['collected_total'] ?? '0.00'), $this->pctLabel($comparison['collected']['delta_percentage'] ?? null)],
    ...
    private function buildPaymentMethodsSheet(Spreadsheet $spreadsheet, array $report, string $hospital, string $rtn): void
    {
        ...
        $methods = $report['payment_methods'] ?? [];
    ...
    private function buildServicesSheet(Spreadsheet $spreadsheet, array $report, string $hospital, string $rtn): void
    {
        ...
        foreach (($report['services']['top_by_amount'] ?? []) as $service) {
        ...
        foreach (($report['services']['top_by_quantity'] ?? []) as $service) {
            $sheet->setCellValue("A{$row}", $this->safeText($service['service'] ?? ''));
            $sheet->setCellValue("B{$row}", $this->safeText($service['category'] ?? ''));
            $sheet->setCellValue("C{$row}", $this->quantityValue($service['quantity'] ?? '0'));
            $sheet->setCellValue("D{$row}", $this->moneyFloat($service['total'] ?? '0.00'));
        ...
        foreach (($report['services']['by_category'] ?? []) as $cat) {
        ...
        foreach (($report['services']['by_area'] ?? []) as $area) {
    ...
    private function buildCashSessionsSheet(Spreadsheet $spreadsheet, array $report, string $hospital, string $rtn): void
    {
        ...
        foreach (($report['cash_sessions'] ?? []) as $session) {
            ...
            $sheet->setCellValue("F{$row}", $session['counted_cash'] !== null ? $this->moneyFloat((string) $session['counted_cash']) : null);
            $sheet->setCellValue("G{$row}", $session['difference'] !== null ? $this->moneyFloat((string) $session['difference']) : null);
    ...
    private function buildPendingSheet(Spreadsheet $spreadsheet, array $report, string $hospital, string $rtn): void
    {
        ...
        $this->applyDataRowStyle($sheet, 'E'.($row - count($pending['items'] ?? [])).':E'.$row, '#,##0.00');
        $this->applyDataRowStyle($sheet, 'F'.($row - count($pending['items'] ?? [])).':F'.$row, '#,##0.00');
    ...
    private function buildVoidsSheet(Spreadsheet $spreadsheet, array $report, string $hospital, string $rtn): void
    {
        ...
        foreach (($report['voids_and_reversals'] ?? []) as $rowData) {
            ...
            $sheet->setCellValue("D{$row}", $this->moneyFloat((string) ($rowData['amount'] ?? '0.00')));
    ...
    {
        return ExcelSafe::value((string) ($value ?? ''));
    }
