...
| `git diff --check` | FAIL | Bloqueado por trailing whitespace/CRLF en `catalogo_servicios_inicial.csv`, cambio concurrente fuera de F21-H/I/J. |
...
Bloqueante de higiene para commit: limpiar o separar el cambio concurrente de `catalogo_servicios_inicial.csv`, porque actualmente hace fallar `git diff --check` global.
