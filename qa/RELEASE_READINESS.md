...
Estado general: LISTO PARA DEMO CONTROLADA.
...
- `php artisan test --colors=never`: OK, 101 tests / 523 assertions.
...
- `C:\Program Files\Git\usr\bin\bash.exe scripts/quality_gate.sh`: OK; re-ejecuto backend y frontend completos. `phpstan` se omitio porque no esta instalado.
...
- Demo credentials existen solo en `local`/`testing` y estan cubiertas por test.
