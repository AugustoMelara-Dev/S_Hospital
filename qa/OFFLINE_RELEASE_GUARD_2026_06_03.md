...
- Verify that `scripts\assert_offline_release_clean.ps1` rejects the current local `offline-release` package after the startup/repair, operator-manual, backup/restore-doc, installation-doc and evidence validators were added.
...
- Blocking issue count: 22 in the latest handoff smoke with `-RequireCurrentCommit`; 21 in the direct guard run without commit validation.
