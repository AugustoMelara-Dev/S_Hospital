...
- Confirm that final handoff exposes the guided `-InitializeProofFiles` switch and the installation guide documents it.
...
- `scripts\final_production_handoff.ps1` exposes `-InitializeProofFiles` and calls the safe proof initializer with `ProjectRoot`.
- The installation guide documents final handoff with `-InitializeProofFiles`.
