...
if not exist "%~dp0.env" (
    ...
    powershell -NoProfile -Command "$chars='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'.ToCharArray(); function New-LocalPassword([int]$Length) { $rng=[System.Security.Cryptography.RandomNumberGenerator]::Create(); try { $bytes=[byte[]]::new($Length); $rng.GetBytes($bytes); $sb=[System.Text.StringBuilder]::new($Length); foreach ($b in $bytes) { [void]$sb.Append($chars[$b %% $chars.Length]) }; return $sb.ToString() } finally { $rng.Dispose() } }; $pass=New-LocalPassword 24; $rootpass=New-LocalPassword 24; Set-Content -Path '%~dp0.env' -Value ('DB_PASSWORD='+$pass+[Environment]::NewLine+'DB_ROOT_PASSWORD='+$rootpass)"
)
