...
function ServiceName({ service }: { service: Service }) { const summary = getServiceBillingSummary(service); const code = visibleServiceCode(service); const secondary = [code ? `Código ${code}` : null, summary.reasons[0]].filter(Boolean).join(' · '); return <div className="min-w-0"><WrapText as="span" className="block font-semibold">{service.name}</WrapText>{secondary ? <WrapText as="p" className="mt-0.5 text-xs text-muted-foreground">{secondary}</WrapText> : null}</div>; }
