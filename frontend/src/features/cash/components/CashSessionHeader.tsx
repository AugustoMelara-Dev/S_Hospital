...
export function CashSessionHeader({ canCloseAnyCash, canCreateInvoices, isOwnSession, isLoading, onRefresh, session }: Props) {
  ...
  return <header className="border-b border-border bg-background p-4 sm:p-6">
    ...
    <div className="mt-4 flex flex-wrap items-center gap-3 text-sm"><Tag color={isOpen ? 'green' : 'default'}>{isOpen ? 'Caja abierta' : 'Caja cerrada'}</Tag><p role="status" aria-live="polite">{isLoading ? 'Actualizando estado de caja.' : isOpen && session ? `Abierta ${formatLocalDateTime(session.opened_at)}` : 'No hay una caja abierta actualmente.'}</p>{isOpen && cashier ? <p><strong>{cashier}</strong> · {isOwnSession ? 'Caja propia' : canCloseAnyCash ? 'Supervisión habilitada' : 'Sesión de otro cajero'}</p> : null}</div>
  </header>;
}
