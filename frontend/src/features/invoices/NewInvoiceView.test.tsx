...
describe('NewInvoiceView', () => {
  ...
  it('shows a retryable LAN load error instead of presenting the POS as only cash-closed', async () => {
    ...
    expect((await screen.findAllByRole("heading", { name: /nueva factura/i })).length).toBeGreaterThan(0);
  ...
  it('renders payment form after issuing an invoice without adding reports', async () => {
    ...
    expect(await screen.findByLabelText(/buscar por nombre, categor�a o c�digo/i)).toBeInTheDocument();
    expect(screen.queryByLabelText(/scanner usb o c�digo manual/i)).not.toBeInTheDocument();
    fireEvent.change(screen.getByLabelText(/buscar por nombre, categor�a o c�digo/i), {
  ...
  it('opens the issued institutional receipt PDF after registering payment', async () => {
    ...
    fireEvent.change(await screen.findByLabelText(/buscar por nombre, categor�a o c�digo/i), {
  ...
  it('does not add a cached service when scanner lookup has no backend match', async () => {
    ...
    fireEvent.change(await screen.findByLabelText(/scanner usb o c�digo manual/i), {
      target: { value: 'CACHED-001' },
    });
    fireEvent.click(screen.getByRole('button', { name: /escanear/i }));

    expect((await screen.findAllByText(/no se encontr� servicio activo para este c�digo/i)).length).toBeGreaterThan(0);
  ...
  it('shows a clear scanner error when the code does not exist', async () => {
    ...
    expect((await screen.findAllByText(/no se encontr� servicio activo para este c�digo/i)).length).toBeGreaterThan(0);
    await waitFor(() => {
      expect(screen.getByLabelText(/scanner usb o c�digo manual/i)).toHaveFocus();
    });
  });
  ...
  it('submits scanner lookup only once while the lookup is pending', async () => {
    ...
    const scannerInput = await screen.findByLabelText(/scanner usb o c�digo manual/i);
    ...
    await waitFor(() => expect(onStatus).toHaveBeenCalledWith('Servicio agregado por c�digo: Glucosa Scanner.'));
  });
  ...
  it('shows void reason confirmation for users with invoice void permission', async () => {
    ...
    expect(await screen.findByLabelText(/motivo de anulaci�n/i)).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText(/motivo de anulaci�n/i), {
  ...
  it('shows pending fiscal configuration on receipts without real authorization data', () => {
    ...
    expect(screen.getAllByText(/configuraci�n pendiente/i)).toHaveLength(3);
  ...
  it('reuses the same invoice idempotency key when retrying after a lost response', async () => {
    ...
    (await screen.findAllByRole("heading", { name: /nueva factura/i })).length;
  ...
  describe('dialysis prescription gating', () => {
    it('allows a user with permission to mark a dialysis prescription and estimates 0.00', async () => {
      ...
      fireEvent.click(screen.getByRole('button', { name: /confirmar emisi�n/i }));
