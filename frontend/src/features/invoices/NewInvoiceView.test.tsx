...
describe('NewInvoiceView', () => {
  ...
  it('renders payment form after issuing an invoice without adding reports', async () => {
    ...
    expect(screen.queryByLabelText(/lector usb o codigo manual/i)).not.toBeInTheDocument();
  ...
  it('rejects inactive services returned by scanner lookup', async () => {
    ...
    fireEvent.change(await screen.findByLabelText(/lector usb o codigo manual/i), {
  ...
  it('shows a clear scanner error when the code does not exist', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText(/lector usb o codigo manual/i)).toHaveFocus();
    });
  });
