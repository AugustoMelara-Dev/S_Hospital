  ...
  function NativeSelect({ className, ...props }, ref) {
    return (
      <select
        ...
        className={cn(
          'flex min-h-10 w-full rounded-md border border-input bg-card px-3 py-2 text-sm text-foreground outline-none shadow-sm',
...
export function SelectTrigger({ className, children, ...props }: SelectPrimitive.SelectTriggerProps) {
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        'flex min-h-10 w-full items-center justify-between gap-2 rounded-md border border-input bg-card px-3 py-2 text-sm text-foreground outline-none shadow-sm',
        'transition-[background-color,border-color,box-shadow] duration-150 ease-out',
        'focus-visible:border-ring focus-visible:bg-card focus-visible:ring-2 focus-visible:ring-ring/35 focus-visible:ring-offset-2 focus-visible:ring-offset-background',
        'aria-[invalid=true]:border-destructive aria-[invalid=true]:ring-2 aria-[invalid=true]:ring-destructive/20',
        'disabled:cursor-not-allowed disabled:opacity-60',
    ...
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDown data-icon aria-hidden="true" className="h-4 w-4 opacity-50" />
    ...
    >
      <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <Check data-icon className="h-4 w-4" aria-hidden="true" />
        </SelectPrimitive.ItemIndicator>
      </span>
