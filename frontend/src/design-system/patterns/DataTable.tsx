  ...
  return <div className="min-w-0">
    ...
    <div className={renderMobileRow ? 'hidden overflow-x-auto rounded-lg border border-border md:block' : 'overflow-x-auto rounded-lg border border-border'}>
      <Table aria-label={ariaLabel}>
        ...
        <TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.id} tabIndex={onRowClick ? 0 : undefined} className={onRowClick ? 'cursor-pointer' : undefined} onClick={() => onRowClick?.(row.original)} onKeyDown={(event) => { if (onRowClick && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); onRowClick(row.original); } }}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id} data-numeric={columnIsNumeric(cell.column.columnDef.meta) ? 'true' : undefined}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}</TableBody>
      </Table>
    </div>
  </div>;
}
