...
function conflictSafeMessage(message: string): string {
  ...
  if (/factura|invoice|pago|payment|duplic|already|ya registrada|ya registrado/.test(normalized)) {
    return 'La factura o el pago ya cambio de estado. Revise Historial antes de repetir la operación.';
  }
    ...
    const send = async (): Promise<Response> => {
      ...
      try {
        ...
        if (err instanceof DOMException && err.name === 'AbortError') {
          recordApiIssue(
            new ApiError(
              `La operación '${method} ${path}' excedio ${timeoutMs / 1000}s sin respuesta del servidor local. Revise la red.`,
              0,
            ),
