...
const reportPath = resolve(process.env.E2E_V1_2_FULL_A11Y_REPORT_PATH ?? '../qa/production-audit/v1-2-full-a11y-report.json');
...
for (const viewport of smokeViewports) {
  test(`v1.2 full UI a11y matrix passes at ${viewport.name}`, async ({ page }) => {
...
function systemStatusSummary() {
  return {
    ...
    checks: [
      { code: 'DATABASE_CONNECTED', label: 'Base de datos', status: 'validated', detail: 'MySQL/MariaDB local conectado.' },
      { code: 'LAN_READY', label: 'Red local', status: 'validated', detail: 'Clientes acceden por IP LAN.' },
      { code: 'BACKUP_RECENT', label: 'Respaldos', status: 'validated', detail: 'Ultimo respaldo disponible.' },
    ],
