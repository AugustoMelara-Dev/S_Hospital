...
async function installApiMocks(page: Page) {
  ...
  await page.route((url) => url.pathname.startsWith('/api/'), async (route) => {
    ...
    if (path === '/api/auth/session') {
      return json(route, { data: isLogged ? adminUser : null });
    }
    if (path === '/api/auth/me') {
      return isLogged ? json(route, { data: adminUser }) : json(route, { message: 'Unauthenticated.' }, 401);
    }
...
async function findVisibleUnnamedControls(page: Page) {
  return page.locator('button, a[href], [role="button"], [role="link"], [role="menuitem"]').evaluateAll((elements) => {
